...
class WorkPackageRoutingMode(StrEnum):
    """Deterministic execution shape selected from real delivery boundaries."""

    SINGLE = "single_package"
    MULTI = "multi_package"
...
class WorkPackageRoutingAudit(BaseModel):
    """Explain why planning kept work together or split it into packages.

    This is generated by the platform after validating the plan; it is never an
    untrusted model instruction and does not require an additional model call.
    """

    model_config = ConfigDict(extra="forbid", frozen=True)

    mode: WorkPackageRoutingMode
    reasons: tuple[str, ...] = Field(min_length=1, max_length=8)
    delivery_layers: tuple[str, ...] = Field(max_length=4)
    package_count: int = Field(ge=1, le=32)
